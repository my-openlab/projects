...
std::pair<std::string, std::string> getDmpFileArguments(int argc, char* argv[]) {
    std::pair<std::string, std::string> dmpFiles;
    // passed via Makefile: PLUSARGS ?= --dmpfile $(TRACE_DIR) $(TRACE_FILE)
    for (int i = 1; i < argc - 2; ++i) {
        if (std::string(argv[i]) == "--dmpfile") {
            dmpFiles.first = argv[i + 1];
            dmpFiles.second = argv[i + 2];
            break;
        }
    }
...
int main(int argc, char** argv) {
    ...
    auto dmpFiles = getDmpFileArguments(argc, argv);
    std::string  dirname = dmpFiles.first.empty() ? ".": dmpFiles.first;
    std::string  filename = dmpFiles.second.empty() ? "dump": dmpFiles.second;
    
#if VM_TRACE_FST
    std::unique_ptr<VerilatedFstC> tfp(new VerilatedFstC);
    top->trace(tfp.get(), 99);
    Verilated::mkdir(dirname.c_str());
    std::string dump_path = dirname + "/" + filename + ".fst";
    tfp->open(dump_path.c_str());
#else
    std::unique_ptr<VerilatedVcdC> tfp(new VerilatedVcdC);
    top->trace(tfp.get(), 99);
    Verilated::mkdir(dirname.c_str());
    std::string dump_path = dirname + "/" + filename + ".vcd";
    tfp->open(dump_path.c_str());
#endif
#endif
